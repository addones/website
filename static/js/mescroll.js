...
function MeScroll(mescrollId, options) {
	this.isScrollBody = (!mescrollId||mescrollId=="body") ; //滑动区域是否为body
	this.scrollDom = this.isScrollBody ? document.body : document.getElementById(mescrollId); //MeScroll的滑动区域
...
MeScroll.prototype.initDownScroll = function() {
	...
	me.downparent = me.optDown.warpId ? document.getElementById(me.optDown.warpId) : me.scrollDom;
	if (me.optDown.warpId) {
		me.downparent.appendChild(me.downwarp);
	} else{
		me.downparent.insertBefore(me.downwarp, me.scrollDom.firstChild);
	}
	
	//鼠标或手指的按下事件
	me.touchstartEvent = function(e) {
		if(me.isScrollTo) e.preventDefault(); //如果列表执行滑动事件,则阻止事件,优先执行scrollTo方法
		me.startTop = me.getScrollTop(); //此时列表到顶部的距离,鼠标或手指离开时制空,避免直接触发move事件;
	...
	me.touchmoveEvent = function(e) {
		//列表在顶部且不在加载中
		if(me.startTop != null && me.getScrollTop() <= 0 && !me.isDownScrolling && (!me.isUpScrolling || (me.isUpScrolling && me.optUp.isBoth)) && !me.optDown.isLock) {
	...
	me.scrollDom.addEventListener("mousemove", me.touchmoveEvent); //PC端鼠标事件
...
MeScroll.prototype.initUpScroll = function() {
	...
	me.upparent = me.optUp.warpId ? document.getElementById(me.optUp.warpId) : me.scrollDom;
	me.upparent.appendChild(me.upwarp);
	...
	me.scrollEvent = function() {
		...
		if(!me.isUpScrolling && (!me.isDownScrolling || (me.isDownScrolling && me.optDown.isBoth))) {
			...
			if(!me.optUp.isLock) {
				var toBottom = me.scrollDom.scrollHeight - me.getClientHeight() - scrollTop; //滚动条距离底部的距离
...
/*获取滚动容器的高度*/
MeScroll.prototype.getClientHeight = function(y) {
	if (this.isScrollBody) {
...
MeScroll.prototype.loadFull = function() {
	var me = this;
	if(me.optUp.loadFull.use && !me.optUp.isLock && me.scrollDom.scrollHeight <= me.getClientHeight()) {
		setTimeout(function() {
			//延时之后,还需再判断一下高度,因为可能有些图片在延时期间加载完毕撑开高度
			if(me.scrollDom.scrollHeight <= me.getClientHeight()) me.triggerUpScroll();
...
MeScroll.prototype.scrollTo = function(y, t) {
	...
	var count = t / rate; //次数
	var me = this;
	var maxY = me.scrollDom.scrollHeight - me.getClientHeight(); //y的最大值
	if(y < 0) y = 0; //不可小于0
	if(y > maxY) y = maxY; //不可超过最大值
	var diff = me.getScrollTop() - y; //差值 (可能为负值)
	if(diff == 0) return;
	...
	var scrollTimer = window.setInterval(function() {
		if(i < count) {
			if(i == count - 1) {
				me.setScrollTop(y); //最后一次直接设置y,避免计算误差
			} else {
				var topY=me.getScrollTop()-step;
				me.setScrollTop(topY);
			}
			i++;
		} else {
			me.isScrollTo = false;
...
MeScroll.prototype.destroy = function() {
	...
	if(me.optDown.use){
		...
		me.downparent&&me.downparent.removeChild(me.downwarp);
	}
	//移除上拉布局,移除事件
	if(me.optUp.use){
		if (me.isScrollBody) {
		...
		me.upparent&&me.upparent.removeChild(me.upwarp);
	}
}